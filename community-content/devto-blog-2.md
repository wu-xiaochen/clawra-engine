...
在 clawra 中，Agent 可以在运行时学习新的推理规则，并**立即体现在推理行为中**：
...
Mem0 负责"记住用户偏好和历史交互"，clawra 负责"基于结构化知识进行可信赖的推理"。
...
如果你在用 Mem0，试试用它做记忆层，在上面构建一个 clawra 推理层——你会发现你的 Agent 从"能回答问题"变成了"能解决复杂问题"。
